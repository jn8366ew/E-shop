...
export const get_item_total = () => async dispatch => {
    if (localStorage.getItem('access')) {
        ...
        let total = 0;
        if (localStorage.getItem('cart')) {
            // localStorage로 부터 getItem를 갖는다 -> 배열오브젝트 -> JSON으로 변환 -> 배열의 길이 체크
...
export const empty_cart = () => async dispatch => {
    if (localStorage.getItem('access')) {
        ...
        try {
            ...
            if (res.status === 200) {
                dispatch({
                    type: EMPTY_ITEM_SUCCESS
                });
            } else {
                dispatch({
                    type: EMPTY_ITEM_FAIL
                });
            }
        } catch (err) {
            dispatch({
